...
class InceptionNet(nn.Module):
    def __init__(self, num_classes=51):
        ...
        self.st_3b = SpatialTransformBlock(num_classes, 32, 256 * 3)  # num_classes, pooling_size, channels=768
        self.st_4d = SpatialTransformBlock(num_classes, 16, 256 * 2)  # num_classes, pooling_size, channels=512
    ...
    def forward(self, input):
        ...
        main_feat = self.global_pool(feat_5b).view(bs, -1)  # 1024
        main_pred = self.finalfc(main_feat)  # 51

        fusion_5b = self.latlayer_5b(feat_5b)  # 256*8*4
        fusion_4d = self._upsample_add(fusion_5b, self.latlayer_4d(feat_4d))  # 512*16*8
        fusion_3b = self._upsample_add(fusion_4d, self.latlayer_3b(feat_3b))  # 768*32*16

        pred_3b = self.st_3b(fusion_3b)  # 51
        pred_4d = self.st_4d(fusion_4d)  # 51
        pred_5b = self.st_5b(fusion_5b)  # 51
